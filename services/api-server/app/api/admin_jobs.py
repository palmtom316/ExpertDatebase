...
def reprocess(req: ReprocessRequest) -> dict:
    runtime_config = apply_runtime_defaults(
        {
        "ocr_provider": (req.ocr_provider or "").strip().lower(),
        "ocr_api_key": (req.ocr_api_key or "").strip(),
        "ocr_model": (req.ocr_model or "").strip(),
        "ocr_base_url": (req.ocr_base_url or "").strip(),
        ...
        "embedding_dimensions": (req.embedding_dimensions or "").strip(),
