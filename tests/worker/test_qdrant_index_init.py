...
def test_ensure_payload_indexes_posts_index_requests(m_put: Mock) -> None:
    ...
    assert any(spec.get("field_name") == "clause_node_id" for spec in DEFAULT_PAYLOAD_INDEXES)
    assert any(spec.get("field_name") == "clause_parent_id" for spec in DEFAULT_PAYLOAD_INDEXES)
    assert any(spec.get("field_name") == "clause_level" and spec.get("field_schema") == "integer" for spec in DEFAULT_PAYLOAD_INDEXES)
